...
export let nextStack={
    ...
    store:async function (){
        ...
        if(this.stack.length==0){
            ...
            let response = await fetch(`./API/employee/find.php?empid=${empid.value}`);            
...
export let prevStack={
    ...
    store:async function (){
        ...
        if(this.stack.length==0){
            ...
            let response = await fetch(`./API/employee/findprev.php?empid=${empid.value}`);
            let result = await response.json();
            this.stack=result
        }
...
export async function recordNavigation(e){
    let data;
    let currentStudent= Object.fromEntries( new FormData(empForm).entries())
    setSearch()
...
async function getFirstLastRecord(){   
    let response = await fetch(`./API/employee/init.php`)
    let result = await response.json();
    // console.log(result);
    return result
