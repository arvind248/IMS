...
export let nextStack={
    ...
    store:async function (){
        ...
        if(this.stack.length==0){
            ...
            let response = await fetch(`./API/student/find.php?rollno=${rollno.value}`);            
...
export let prevStack={
    ...
    store:async function (){
        ...
        if(this.stack.length==0){
            ...
            let response = await fetch(`./API/student/findprev.php?rollno=${rollno.value}`);
            let result = await response.json();
            this.stack=result
        }
...
async function getFirstLastRecord(){   
    let response = await fetch(`./API/student/init.php`)
    let result = await response.json();
    // console.log(result);
    return result
