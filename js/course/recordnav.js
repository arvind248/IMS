...
export let nextStack={
    ...
    store:async function (){
        ...
        if(this.stack.length==0){
            ...
            let response = await fetch(`./API/course/find.php?name=${name.value}`);            
...
export let prevStack={
    ...
    store:async function (){
        ...
        if(this.stack.length==0){
            ...
            let response = await fetch(`./API/course/findprev.php?name=${name.value}`);
            let result = await response.json();
            this.stack=result
        }
...
export async function recordNavigation(e){
    let data;
    let currentStudent= Object.fromEntries( new FormData(courseForm).entries())
    setSearch()
...
async function getFirstLastRecord(){   
    let response = await fetch(`./API/course/init.php`)
    let result = await response.json();
    // console.log(result);
    return result
